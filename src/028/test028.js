...
function createQuad(x, y, width, height) {
  var TL = new Vec2(x, y);
  var TR = new Vec2(x + width, y);
  var BL = new Vec2(x, y + height);
  var BR = new Vec2(x + width, y + height);
  var UVTL = new Vec2(0, 1);
  var UVTR = new Vec2(1, 1);
  var UVBL = new Vec2(0, 0);
  var UVBR = new Vec2(1, 0);
  return {
    position: new Float32Array([TL.x, TL.y, TR.x, TR.y, BL.x, BL.y, BL.x, BL.y, TR.x, TR.y, BR.x, BR.y]),
    uvs: new Float32Array([UVTL.x, UVTL.y, UVTR.x, UVTR.y, UVBL.x, UVBL.y, UVBL.x, UVBL.y, UVTR.x, UVTR.y, UVBR.x, UVBR.y])
  };
}
