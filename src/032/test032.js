...
class Quad {
  constructor(x, y, z, width, height) {
    ...
    _defineProperty(this, "_rotation", void 0);
...
var cameraPosition = new Vec3(400, 256, -256);
var lookPosition = new Vec3(400, 200, 0);
var orientation = new Vec3(0, 1, 0);
var viewMatrix = LookAt(cameraPosition, lookPosition, orientation);
var p = Perspective(180, app.width / app.height, 0, 1000);
var projectionMatrix = Multiply(p, viewMatrix);
var sub = app.createUniformBuffer([app.gl.FLOAT_MAT4]);
sub.set(0, projectionMatrix.getArray()).update();
var UVTL = new Vec2(0, 1);
var UVTR = new Vec2(1, 1);
var UVBL = new Vec2(0, 0);
var UVBR = new Vec2(1, 0);
var quads = [];
var max = 200;
...
for (var i = 0; i < max; i++) {
  ...
  var quad = new Quad(x, y, 0, 64, 64);
  quads.push(quad);
}

var size = 4;
var dataTA = new Float32Array(size * (max * 29));
var offset = 0;
quads.forEach(quad => {
...
var buffer = app.createInterleavedBuffer(size * 5, dataTA);
...
ImageFile('sprites', '../assets/box-item-boxed.png').load().then(file => {
  ...
  quads.forEach(quad => {
    var x = Math.floor(Math.random() * app.width);
    var y = Math.floor(Math.random() * app.height);
    var z = -200 + Math.floor(Math.random() * 400);
...
//# sourceMappingURL=test032.js.map
