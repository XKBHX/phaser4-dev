...
function AddToDOM(a,b){var c;if(b)"string"==typeof b?c=document.getElementById(b):"object"==typeof b&&1===b.nodeType&&(c=b);else if(a.parentElement)return a;return c||(c=document.body),c.appendChild(a),a}

function isCordova(){return window.hasOwnProperty("cordova")}

function DOMContentLoaded(a){if("complete"===document.readyState||"interactive"===document.readyState)return void a();var b=()=>{document.removeEventListener("deviceready",b,!0),document.removeEventListener("DOMContentLoaded",b,!0),window.removeEventListener("load",b,!0),a();};document.body?isCordova()?document.addEventListener("deviceready",b,!0):(document.addEventListener("DOMContentLoaded",b,!0),window.addEventListener("load",b,!0)):window.setTimeout(b,20);}

class Game{constructor(a){_defineProperty(this,"canvas",void 0),_defineProperty(this,"context",void 0),_defineProperty(this,"isBooted",!1),_defineProperty(this,"isRunning",!1),_defineProperty(this,"_initCallback",void 0),this._initCallback=a,DOMContentLoaded(()=>this.boot());}boot(){console.log("Phaser 4.0.0-alpha.3"),this.isBooted=!0,this.createDebugCanvas(),AddToDOM(this.canvas),this._initCallback(this);}createDebugCanvas(){var a=0<arguments.length&&arguments[0]!==void 0?arguments[0]:800,b=1<arguments.length&&arguments[1]!==void 0?arguments[1]:600;this.canvas=document.createElement("canvas"),this.canvas.width=a,this.canvas.height=b,this.context=this.canvas.getContext("2d"),this.context.fillStyle="#2d2d2d",this.context.fillRect(0,0,a,b);}drawImage(a){var b=1<arguments.length&&arguments[1]!==void 0?arguments[1]:0,c=2<arguments.length&&arguments[2]!==void 0?arguments[2]:0;this.context.drawImage(a,b,c);}draw(a){this.context.fillStyle="#ff0000",this.context.fillText(a,10,40),this.context.fillStyle="#0000ff",this.context.fillText(a,10,20),this.context.fillStyle="#ffff00",this.context.fillText(a,10,60);}text(a,b,c){this.context.fillStyle="#00ff00",this.context.font="16px Courier",this.context.fillText(c,a,b);}}
...
new Game(a=>{a.draw("Phaser 4 Test 001"),ImageFile("logo","../assets/logo.png").load().then(b=>{for(var e=0;10>e;e++){var c=700*Math.random(),d=500*Math.random();a.drawImage(b.data,c,d);}});});
