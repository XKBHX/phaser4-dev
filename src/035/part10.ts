...
{
    //  Using the Transform and a Quad class to handle the Matrix2D math
    //  These files are external, but all the WebGL is still handled in this single file
    //  Renderer now batching (all quads in a single array, needs splitting-up into sub-buffers soon)
    ...
    for (let i = 0; i < max; i++)
    {
        ...
        let quad = new Quad(x, y, 128, 128, r, g, b, 0.3);
