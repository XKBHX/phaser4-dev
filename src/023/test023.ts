...
void main() {
    uv = inUV;
    fragCoord = vec4(inPosition, 0.0, 1.0);
...
void main() {
    ...
    float px = (fragCoord.x * 0.5 + 0.5) * 1024.0;
    float py = (fragCoord.y * 0.5 + 0.5) * 1024.0;

    if (px >= 0.0 && px < 256.0 && py >= 0.0 && py < 353.0)
    {
        fragColor = texture(tex, uv);
    }
    else
    {
        fragColor = vec4(fragCoord.x * 0.5 + 0.5, fragCoord.y * 0.5 + 0.5, 1.0, 1.0);
    }
}
`;
