...
function buildDraw(texture) {
  ...
  for (var i = 0; i < max; i++) {
    var x = Math.floor(Math.random() * (app.width - width));
    var y = Math.floor(Math.random() * (app.height - height));
    var TL = app.getXY(x, y);
    data.push(TL.x, TL.y);
  }

  var vertices = app.createVertexBuffer(app.gl.FLOAT, 2, new Float32Array(data));
  var vertexArray = app.createVertexArray().vertexAttributeBuffer(0, vertices);
  data.length = 0;
  var dc = app.createDrawCall(program, vertexArray).texture('tex', texture);
